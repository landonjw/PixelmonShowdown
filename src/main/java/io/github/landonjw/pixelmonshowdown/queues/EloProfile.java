...
public class EloProfile {
    ...
    private final Boolean IS_PERSISTENT = DataManager.getConfigNode().getNode("Elo-Management", "K-Factor", "K-Factor-Persistent").getBoolean();
    private final double K_FACTOR_LOW_ELO = DataManager.getConfigNode().getNode("Elo-Management", "K-Factor", "K-Factor-Low-Elo").getDouble();
    private final double K_FACTOR_MID_ELO = DataManager.getConfigNode().getNode("Elo-Management", "K-Factor", "K-Factor-Mid-Elo").getDouble();
    private final double K_FACTOR_HIGH_ELO = DataManager.getConfigNode().getNode("Elo-Management", "K-Factor", "K-Factor-High-Elo").getDouble();
    private final double K_FACTOR_PERSISTENT = DataManager.getConfigNode().getNode("Elo-Management", "K-Factor", "K-Factor-Persistent-Value").getDouble();
    private final int ELO_FLOOR = DataManager.getConfigNode().getNode("Elo-Management", "Elo-Range", "Elo-Floor").getInt();
    private final int LOW_ELO_RANGE = DataManager.getConfigNode().getNode("Elo-Management", "Elo-Range", "Low-Elo-Range").getInt();
    private final int HIGH_ELO_RANGE = DataManager.getConfigNode().getNode("Elo-Management", "Elo-Range", "High-Elo-Range").getInt();
    ...
    private double getExpectedOutcome(int oppElo){
        double outcome = 1 / (1 + Math.pow(10, (oppElo - elo)/400.0));
        return outcome;
    }
}
